...
For instance, if we put animal in different incubators, we can simply add an `incubator` column.
...
Once your metadata is ready, it can be used as a query to **import the matching data**.
...
* Adding an `id` column to the metadata. This will be a unique identifier for each indididual (it generally contains datetime, machine name an region id)
