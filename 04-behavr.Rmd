...
As we have seen in the [previous section](metadata.html), metadata are crucial for proper statistical analysis of the experiment **data**. That is, a long time series of recorded **variables** such as position, orientation and number of beam crosses, for each individual.
...
A `behavr` table is, indeed, formed internally by two tables: the metadata table and the data table, both are related by the id field. (This squematic is shown in the figure).

For most purposes, you can use a `behavr` table just like a `data.table`.
Therefore, we recommend to have a look at the [introduction to `data.table`](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html) for further details!
...
Again, we recommend to have a look to [introduction to `data.table`](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html) so this part of the tutorial will be easy to understand. 
This table is a short overview of permited operations in `behavr` tables, where DT or dt is the name of our `behavr` table:
...
The `behavr` package has a set of functions to make toy data. This is not only useful for the tutorial but also to test functions and plots.
In order to have a look at a `behavr` object, lets create one.
First, we make some fake metadata (always needed to create a `behavr` table):
...
metadata
```
This metadata describes a theoretical experiment with 10 animals (1:10), where 5 are males and the other 5 are females, they are expose to 2 conditions (A and B).
...
dt
```
As you can see, when we print `dt`, our `behavr` table, we have two tables: `METADATA` and `DATA`.
...
note: paste is a r command to link strings by the desired separation character.
...
summary(dt)
```
When individuals are removed, metadata is automatically updated. In effect, we removed males from both data and metadata. This operation cannot be undone, so be carefull to delete data. You can create a new `beharv` table with a new name, if you override it (like in the example) and you want to recover it, you will have to load it again.
...
Now, in order to perform statistics, we would like to merge our summaries to the metadata. This way we will end up with only one `data.table`
That is, we want to **rejoin** them or autocomplete our summaries with the metadata:
