...
Typically, metavariables are defined as extra *columns a the metadata file*, but they can also be computed.
...
```{r}
library(behavr)
metadata <- data.frame( id = sprintf("toy_experiment|%02d",1:10),
                        condition = c("A", "B") )
metadata
```
...
The former has only one row per animal and looks very much like metadata, with an extra column named `id`.
