...
In order to work with the data the last step is to load it into a [behavr](behavr.html) structure. To do that simply use `load_dam2` function (as shown below). This function will store all data in dt (or any other given name)
...
ggetho(dt[xmv(replicate) == 1 ], aes(z=asleep)) +
      stat_tile_etho() +
      stat_ld_annotations()
```
