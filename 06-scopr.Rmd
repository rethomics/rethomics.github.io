...
A zip containing data for this tutorial is available on [zenodo](https://zenodo.org/record/826874)
...
Everytime an ethoscope is started, one `.db` new file is created.
...
In addition, in the end of the experiment, we also collected flies and **measured a the relative expression of our candidate gene**.

Ultimatly, we would like to:

* Analyse the effect of the drug on activity
...
We defined also custom columns -- i.e. metavariable  (note that you could define many, with arbitrary names):
...
* Have to download all the data from all users at all time (very ineficient)
...
It will not download datafile everytime (unless new data is available upstream).
...
However, this has sevral caveats:
...
It shows us the metadata, and  the firt and last few lines of actual data.
...
In the context of activity and sleep analysis, we can describe movement in windows of 10s ([techinical details here](http://journals.plos.org/plosbiology/article/file?type=supplementary&id=info:doi/10.1371/journal.pbio.2003026.s005)).
...
We have several variable. the important ones are:
...
In other words, t0 is when you click on the "start tracking" button.
...
For instance, according to our computer, you may take half an hour to load data from 500 animals x 1 week.
In your daily life, you will often need to close R and open it again to analyse things differently, or simply change simple things on some figures. You really don't want to wait too long in this sort of situations.

To address this issue, `load_ethoscope` uses [caching](https://en.wikipedia.org/wiki/Cache_(computing)).
In other words, whenever you load any data, it stores a snapshot on the disk. then, the *next time* you load the data, it takes directly the snapshot. This way you will go one or two orders of magnitudes faster!

The first time, everything will take long:
...
This allows us to spot possible outliers/missing data/artefact and try to understand what to do whith them.
...
We can show the how **sleep fraction and expression level are decorelated**
