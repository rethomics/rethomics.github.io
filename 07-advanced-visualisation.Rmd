...
This section will explain furhter how this packakescan be used to produce flexible plots and how it integrates with `ggplot2`.

[ggplot2](http://ggplot2.org/) is one of the most popular visualisation tool, and an unavoidable `R` package. 
It implements the powerful concepts of the "Grammar of graphics".
...
dt <- toy_activity_data(metadata, seed=3)
```
...
In this respect, it works very much like `ggplot()`, but it also preprocesses the data.
...
By defaut, each pixel is the *mean* (`summary_FUN = mean`, in `ggetho()`), over 30min (`summary_time_window = mins(30)`, in `ggetho()`). 
...
For instance let us compute the overall average fraction of time spent moving to then sort individuals from low to high movers:

First, we add a new metavariable (`mean_moving`):
```{r}
# the average time spent moving per 1000
...
Now, we can express a new relationship where we show the *interaction* between our custom variable and id:

```{r}
pl <- ggetho(dt, aes(x=t, y=interaction(id, mean_moving, sep = " : "), z=moving)) + stat_tile_etho()
pl
```
...
```{r}
pl <- ggetho(dt, aes(x=t, y=interaction(id, mean_moving, sex, sep = " : "), z=moving)) + stat_tile_etho()
pl
```
### Group averages{-}

Sometimes, we also want to aggregate inviduals per group.
For instance, males **average** vs females **average**:
```{r}
pl <- ggetho(dt, aes(x=t, y=sex, z=moving)) + stat_tile_etho()
pl
```
In this context, every row is not an individual anymore, but a group.
...
```{r}
pl <- ggetho(dt, aes(x=t, y=moving)) + stat_pop_etho() + facet_grid(. ~ sex)
pl
```

Of course, you can combine both when you have more than one condition:

```{r}
pl <- ggetho(dt, aes(x=t, y=moving, colour = sex)) + stat_pop_etho() + facet_grid(. ~ condition)
pl
```
...
```{r}
pl <- ggetho(dt, aes(x=t, y=moving)) + stat_pop_etho()+ stat_ld_annotations(ld_colours = c("grey", "black"))
pl
```
...
```{r}
pl <- ggetho(dt, aes(x=t, y=moving)) + stat_ld_annotations(height=1, alpha=0.3, outline = NA) + stat_pop_etho()
pl
```
You can also play with the phase and periods of the annotations (see the documentation).

##Coordinate and scales{-}

As `ggetho` creates regular ggplot object, we can extend them further. For instance, we can change the scales.
For instance, put the y scale as a percentage between 0 and 100:

```{r}
pl <- ggetho(dt, aes(x=t, y=moving)) + stat_pop_etho() + stat_ld_annotations()
pl <- pl + scale_y_continuous(limits = c(0,1), labels = scales::percent)
pl
```
