...
A "spectrograms" is a visualistion of the variation in the period spectrum over time.
The function `spectrogram` in the `zeitgebr`  library can generate a spectrogram for each individual in a `behavr` tables, and return the result as a new `behavr` table with columns for power, period, time ... `ggetho` makes it possible to visualise spectrograms as a heatmap, annotate them, aggregate them by condition.
...
This spectrograms resulting from `spectrogram()` can be vusualised thanks to our special function, `ggspectro()`:
...
Note that, in this example, we facet by condition, but we could also facet by `id`, or average all individuals in one graph. An intesresting feature of such periodograms is that the phase information is discarded. 
In other word, we can average the periodograms of multiple individuals (e.g. two populations) even if they are out of phase (e.g. if they are free running). This represents a clear advantage over double-plotted actograms that are often only relevant on single individual.
