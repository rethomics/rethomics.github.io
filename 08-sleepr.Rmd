...
For instance, we want to remove the begining as animals are acclimatising to their new environment.
In general, we could simply do:


```{r}
dt_curated <- dt_curated[t %between% c(days(1), days(3.5)]
summary(dt_curated)
```

Which means we are only considering data **between** 1 and 3.5 days after the experiment started. 
